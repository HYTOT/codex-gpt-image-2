...
1. 每个 `##` 下必须有 1 到 3 个 `-` 条目。
2. 单条 `-` 可写成中长句，但不要写成长篇散文。
3. 每个任务默认视为独立新任务。
...
2. 再写 `tasks/<任务名>/task_prompt.md`，直接产出合规六段式最终提示词，确保每节 1 到 3 条、默认独立新任务、避免跨任务修订语气。
...
9. 如本次新增或修改任务，`task_prompt.md` 是否在创建阶段已满足六段式标题、每节 1 到 3 条和禁用话术约束。
